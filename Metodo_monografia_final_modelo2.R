...
#ESSE METODO ADICIONA NOVAS VARIAVEIS AO TREINO
#ESTUDO 1 ----------------------------------------------------------------------
...
for(m in 1:100){
  ...
  for(n in 1:nrow(teste)){
    ...
    if(teste$class[n] > 0.9 * 30){
      ...
      treino <- rbind(treino,teste[n,c('x','y','t','cluster')])
      contador <- contador + 1
      if(contador == 10){
        indicador <- T
      }
...
saveRDS(final,'Resultado_Estudo1_1_var.RDS')
...
saveRDS(final,'Resultado_Estudo2_1_var.RDS')
...
saveRDS(final,'Resultado_Estudo3_1_var.RDS')
