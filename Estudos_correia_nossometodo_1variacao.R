...
#APAGAR OBSERVACOES DENTRO DO CILINDRO DO ALARME QUANDO ELE SOAR
...
for(m in 1:10){
  ...
  for(n in 0:(nrow(teste)/n_variaveis)){
    ...
    #SELECIONANDO AS OBSERVACOES (mudar ao numero de variaveis no conjunto teste)
    ...
    for(b in 1:30){
      ...
      svm.model <- svm(treino[c('var1','var2','var3')],y=NULL,
                     scale = TRUE,
